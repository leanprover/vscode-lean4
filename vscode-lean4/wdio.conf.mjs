...
export const config = {
    ...
    async onWorkerStart() {
        ...
        // Windows ships the CLI as `bin\code.cmd`; Linux/macOS as `bin/code`.
        // `execFileSync` will not run a `.cmd` directly (Node refuses since
        // CVE-2024-27980), so on Windows we route through the shell. The
        // arguments here have no shell-meaningful characters, so injection
        // isn't a concern.
        const isWin = process.platform === 'win32'
        const codeCli = path.join(cacheDir, vscodeDir, 'bin', isWin ? 'code.cmd' : 'code')
