...
// Local override for the warm-env timeouts in `helpers/timeouts.ts`. The
// mathlib `lake new`-created project's `lean-toolchain` may pin a version
// that isn't pre-installed in the cached test home, so the first
// `openAndAwaitClient` here can trigger a real toolchain download. Build
// + cache fetch on a fresh runner takes minutes; ten covers a slow CDN.
const TOOLCHAIN_INSTALL_TIMEOUT_MS = 10 * 60_000

// Mathlib-heavy project-init tests: each one fetches the full mathlib history
// and/or its build-artifact cache, so they only run under the network label.
...
describe('Mathlib project initialization (real network)', function () {
        ...
        try {
            await withPrompts(
                ...
                async ({ log }) => {
                    ...
                    try {
                        await openAndAwaitClient(basicUri, TOOLCHAIN_INSTALL_TIMEOUT_MS)
                        await waitForDiagnostics(
                            basicUri,
                            hasInfoIncluding(sentinel),
                            TOOLCHAIN_INSTALL_TIMEOUT_MS,
    ...
    it('`lean4.project.fetchCache` succeeds on a freshly scaffolded Mathlib project', async () => {
        ...
        await openAndAwaitClient(mainUri, TOOLCHAIN_INSTALL_TIMEOUT_MS)
