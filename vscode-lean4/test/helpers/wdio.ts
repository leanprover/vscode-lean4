...
import { DIAGS_TIMEOUT_MS } from './timeouts'
...
// Polls the InfoView WebView for body text matching `predicate`. The
// InfoView's content is gated on the Lean server publishing goals, so its
// budget tracks `DIAGS_TIMEOUT_MS`; reuse the constant so the two move
// together when latency assumptions shift. A timeout signals that the
// content never arrived, not that we couldn't find the InfoView — the
// frame is identified by `document.title`, not by content matching.
export async function waitForInfoView(
    predicate: (text: string) => boolean,
    { label, timeout = DIAGS_TIMEOUT_MS }: { label: string; timeout?: number },
): Promise<void> {
