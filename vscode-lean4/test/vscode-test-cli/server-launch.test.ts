...
import { DIAGS_TIMEOUT_MS, RESTART_DIAGS_TIMEOUT_MS } from '../helpers/timeouts'

describe('Language server — launch and restart', function () {
    this.timeout(DIAGS_TIMEOUT_MS + 60_000)
    ...
    // left behind. Subsequent diagnostics assertions inside each test use the
    // (same-sized but separately-named) `RESTART_DIAGS_TIMEOUT_MS`.
    beforeEach(async () => {
        await openInEditor(uri)
        await waitForDiagnostics(uri, hasError, DIAGS_TIMEOUT_MS, 'initial')
    })
