...
- `network/` — exercises the real Elan + toolchain install flow plus `lake new … math` (Mathlib clone + cache fetch). Run via `npm run test:network`. Runs on the nightly cron / on merge.
...
# Network suite (downloads elan + leanprover/lean4:stable + clones Mathlib, ~200MB+)
...
### Lean nightly coverage

The `test-nightly` job in `.github/workflows/test.yml` runs the same `test:headless` suite against `leanprover/lean4:nightly` instead of `:stable`. It pre-installs the nightly toolchain, sets it as elan's default (so detached / untitled-file tests use it), and rewrites each fixture's `lean-toolchain` from `:stable` to `:nightly` via anchored `sed` — the mock-toolchain fixtures are untouched, and the in-tree fixtures stay on `:stable`, so PR CI is unaffected. Runs on the nightly schedule and `workflow_dispatch` only.

Failures here are more often an upstream Lean change (elaboration order, LSP message shape, error text drift) than an extension bug. Reproduce locally by running `elan default leanprover/lean4:nightly` and applying the same fixture rewrite the CI step uses, then `npm run test:headless`. If the failure is a legitimate upstream regression, file at `leanprover/lean4` rather than working around it in the extension.

### Headless on Linux

Running `npm test` directly opens real VS Code windows that grab focus from your DE for the duration of the suite. The `test:headless`, `test:headless:wdio`, and `test:headless:network` scripts route the inner command through `scripts/headless.mjs`, which on Linux wraps it in `xvfb-run -a` (rendering test instances to a virtual X display — no focus-stealing, no taskbar entries) and on Windows / macOS runs it directly. The Linux path requires `xorg-x11-server-Xvfb` (Fedora) / `xvfb` (Debian/Ubuntu); the Windows / macOS path needs nothing extra. Same `npm run` invocation everywhere.
...
Populated by the vscode-test-cli suite (covers `cli-*` and the `network` label).
...
| `timeouts.*` | `DIAGS_TIMEOUT_MS`, `RESTART_DIAGS_TIMEOUT_MS`, `CLIENT_READY_MS`, `ABBREVIATION_TIMEOUT_MS` — shared constants for warm-environment timing (toolchain pre-installed). Network tests under `test/network/` define their own longer budgets locally. |
