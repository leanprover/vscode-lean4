...
import { addDefaultElanPath, removeElanPath, addToolchainBinPath, isElanDisabled, getTestVersion} from './config';
...
export async function activate(context: ExtensionContext): Promise<Exports> {
    ...
    const defaultToolchain = getTestVersion() ?? 'leanprover/lean4:nightly';
