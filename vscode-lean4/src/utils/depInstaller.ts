...
function dependencyInstallationMethod(p: MissingDependencyInstallationProcedure): DependencyInstallationMethod {
    switch (p.kind) {
            ...
            switch (p.packageManager.kind) {
                case 'Apt':
                    ...
                    return {
                        kind: 'Automatic',
                        shell: 'Unix',
                        script: `pkexec bash -c 'export DEBIAN_FRONTEND=noninteractive; apt-get update -y && apt-get install -y ${missingDeps(p).join(' ')}'`,
        ...
        case 'MacOS':
            const macOSInstallScript = 'xcode-select --install'
            return {
                kind: 'Automatic',
                shell: 'Unix',
