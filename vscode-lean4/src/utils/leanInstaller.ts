...
export class LeanInstaller implements Disposable {
    ...
    async installElan(): Promise<ElanInstallationResult> {
        const r = await this.runOperation<ElanInstallationResult>('Install', async () => {
            const method = this.determineElanInstallationMethod()
    ...
    determineElanInstallationMethod(): ElanInstallationMethod {
        if (process.platform === 'win32') {
            return {
                script: windowsInstallationScript(this.freshInstallDefaultToolchain),
                shell: 'Windows',
            }
        }
        return {
            script: unixInstallationScript(this.freshInstallDefaultToolchain),
            shell: 'Unix',
        }
    }
    ...
    async displayInstallationUnsuccessfulPrompt(result: ExecutionResult) {
        const error =
            ...
            'If you are unable to figure out the issue from the command output below, you can also try running the following manual installation script from a [terminal](command:workbench.action.terminal.new):\n\n' +
            this.determineElanInstallationMethod().script
