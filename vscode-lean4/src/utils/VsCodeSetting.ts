...
export class VsCodeSetting<T> {
    ...
    constructor(
        ...
        this.settingResource = new VsCodeSettingResource(this.id, this.scope, this.target)
    }
    ...
    async set(value: T): Promise<void> {
        ...
        if (this.target !== undefined) {
            ...
            if (
                result &&
                [result.workspaceFolderLanguageValue, result.workspaceFolderValue].some(i => i !== undefined)
            ) {
                target = ConfigurationTarget.WorkspaceFolder
            }
            if (result && [result.workspaceLanguageValue, result.workspaceValue].some(i => i !== undefined)) {
...
class VsCodeSettingResource {
     ...
     */
    private readonly stringifiedSettingValue = computed(() => JSON.stringify(this.resource.current()), {
...
workspace.onDidChangeConfiguration(() => runInAction(() => VsCodeSettingResource.onConfigChange.fire(undefined)))
