...
export class FullInstaller implements Disposable {
    ...
    private async displayManualInstallationPrompt(
        ...
        const prompt = `Please install ${missingDepsSentence(p)} from a [terminal](command:workbench.action.terminal.new) using the script below, restart VS Code and repeat this step to install Lean.\n\n${p.method.script}`
