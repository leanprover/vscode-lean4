...
export class AbbreviationRewriter {
            ...
            workspace.onDidChangeTextDocument(e => {
                ...
                void this.forceReplace(
                    [...this.trackedAbbreviations].filter(
                        abbr =>
                            abbr.finished ||
                            (this.config.eagerReplacementEnabled.get() && abbr.isAbbreviationUniqueAndComplete),
                    ),
                )
            }),
        )
            ...
            window.onDidChangeTextEditorSelection(e => {
                ...
                void this.forceReplace(
                    [...this.trackedAbbreviations].filter(
                        abbr =>
                            !e.selections.some(s =>
                                abbr.range.containsRange(fromVsCodeRange(s, e.textEditor.document).withLength(0)),
                            ),
                    ),
                )
            }),
    ...
    private async replaceAbbreviations(
        replacements: {
            range: Range
            newText: string
            cursorOffset?: number | undefined
    ...
    private updateState() {
        this.textEditor.setDecorations(
            this.decorationType,
            [...this.trackedAbbreviations].map(a => toVsCodeRange(a.range, this.textEditor.document)),
        )
    ...
    private processChange(range: Range, text: string): { affectedAbbr: TrackedAbbreviation | undefined } {
        ...
        if (text === this.config.abbreviationCharacter.get() && !affectedAbbr && !this.dontTrackNewAbbr) {
            const abbr = new TrackedAbbreviation(new Range(range.offset + 1, 0), '', this.abbreviationProvider)
