...
export class DocViewProvider implements Disposable {
    ...
    private async httpGet(uri: Uri): Promise<string> {
        return new Promise<string>(resolve => {
            void axios.get(uri.toString(), { responseType: 'arraybuffer' }).then(response => {
    ...
    async fetch(url?: string): Promise<string> {
        if (url) {
            try {
                ...
                $('p')
                    .text(`Unsupported file type '${fileType}', please `)
                    .append($('<a>').attr('href', url).attr('alwaysExternal', 'true').text('open in browser instead.'))
                return $.html()
            } catch (ex) {
                const $ = cheerio.load('<p>')
            ...
            const books: any = {
                'Theorem Proving in Lean': mkCommandUri(
                    'lean4.docView.openUrl',
                    'https://lean-lang.org/theorem_proving_in_lean4/introduction.html',
                ),
                'Functional Programming in Lean': mkCommandUri(
                    'lean4.docView.openUrl',
                    'https://lean-lang.org/functional_programming_in_lean/',
                ),
                'Mathematics in Lean': mkCommandUri(
                    'lean4.docView.openUrl',
                    'https://leanprover-community.github.io/mathematics_in_lean/',
                ),
                'The Mechanics of Proof': mkCommandUri(
                    'lean4.docView.openUrl',
                ...
                Example: mkCommandUri(
                    'lean4.docView.openExample',
                    'https://github.com/leanprover/lean4-samples/raw/main/HelloWorld/Main.lean',
                ),
            ...
            for (const book of Object.getOwnPropertyNames(books)) {
                body.append(
                    $('<p>').append(
                        $('<a>')
                            .attr('href', books[book] as string)
                            .text(book),
                    ),
                )
            }
...
var side_bar = false; // collapse the side bar menu by default.
        ...
        const mkLink = (command: string, title: string, text: string) =>
            $('<a>')
                .attr('title', title)
