...
// path so the toolchain download survives between runs. The network suite's
...
export function setupTestHome() {
    ...
    if (process.env.LEAN4_TEST_HOME_OVERRIDE) {
        ...
        // Use the `lean4-test-home-` prefix so the network suite's safety check
    ...
    if (fs.existsSync(settingsPath) && !/^\s*version\s*=/m.test(fs.readFileSync(settingsPath, 'utf8'))) {
        // Retry params mirror `test/helpers/fs.ts`. Single-file unlinks are
        // low-risk for the Windows PE-image lock race (no executable here),
        // but consistent retry policy across the codebase is cheap.
        fs.rmSync(settingsPath, { maxRetries: 10, retryDelay: 100 })
    }
    if (!fs.existsSync(settingsPath)) {
        try {
            ...
            if (e.code === 'ENOENT') {
                // No elan on PATH. The network suite (`test:network`) is
                // allowed to start in this state because installing elan
                // is part of what its `elan-install.test.ts` exercises.
                // For other flows it's a misconfiguration, but we don't
                // fast-fail here anymore — a missing elan will surface
                // immediately at the first toolchain-resolving test with
                // a clear "elan/lean not found" error from the extension
                // itself. No need to short-circuit.
