...
if (process.env.LEAN4_TEST_OUTPUT_WIPED !== '1') {
    // Retry params mirror `test/helpers/fs.ts` — see the comment there.
    fs.rmSync(cliOutputDir, { recursive: true, force: true, maxRetries: 10, retryDelay: 100 })
}
...
// The network and vscode-test-cli suites all install a toolchain on first run
// and benefit from a generous default. Leaving them on the 120s common default
// would mean any test that forgets its own `this.timeout` fails the moment
// lake or elan starts a real download.
...
export default defineConfig([
    {
        label: 'network',
        files: 'out/test/network/**/*.test.js',
