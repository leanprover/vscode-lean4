...
export function activate(context: ExtensionContext): void {
    ...
    context.subscriptions.push(
        ...
        commands.registerTextEditorCommand(
            "lean.batchExecute",
            (editor, edit, args) => {
                batchExecuteFile(server, editor, edit, args);
            }
        )
    );
    ...
    context.subscriptions.push(
        languages.registerCompletionItemProvider(
            LEAN_MODE,
            new LeanCompletionItemProvider(server),
            "."
        )
    );
    ...
    context.subscriptions.push(
        languages.registerDefinitionProvider(
            LEAN_MODE,
            new LeanDefinitionProvider(server)
        )
    );
    ...
    function waitStaticServer() {
        ...
        const infoView = new InfoProvider(
            server,
            LEAN_MODE,
            context,
            staticServer
        ...
        // Tactic suggestions
        context.subscriptions.push(
            new TacticSuggestions(server, infoView, LEAN_MODE)
        );
    }
